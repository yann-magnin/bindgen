...
public class BindgenConfig {
	...
	private final Scope<ClassName> exclude;
	...
	public BindgenConfig(ProcessingEnvironment env) {
		...
		this.exclude = this.getBindingExclude();
	}
	...
	public boolean shouldGenerateBindingFor(ClassName name) {
		return this.bindingScope.includes(name) && !this.exclude.includes(name);
	}
	...
	public boolean skipExistingBindingCheck() {
		...
		// javac doesn't like skipping existing bindings , so default to true if
		// in javac
...
//		but references it as ModuleDescriptor.Class2Binding in generated classes
	...
	// Default properties--this is ugly, but I could not get a
	// bindgen.properties to be found on the classpath
